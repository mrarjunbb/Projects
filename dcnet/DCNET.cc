...
void DCNET::SendConfirmation(bool isFirstMessage)
{
    ...
    else
    {
        ...
        for (std::map<ns3::Ipv4Address,int>::iterator it = m_nodeaddr_nodeid.begin() ; it != m_nodeaddr_nodeid.end(); ++it)
        {
            ...
            if(m_my_star_nodeid < node_id) m_my_star_nodeid=node_id;
...
void DCNET::GeneratePublickeys()
{
    ...
    for (std::map<int,ns3::Ipv4Address>::iterator it = m_nodeid_nodeaddr.begin() ; it != m_nodeid_nodeaddr.end(); ++it)
    {
        ...
        {
            if(std::find(m_public_keys_neighbors.begin(), m_public_keys_neighbors.end(), neighbor_nodeid) == m_public_keys_neighbors.end())
            {
                m_public_keys_neighbors.push_back(neighbor_nodeid);
                Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable> ();
...
void DCNET::SendFinalAnnoucementForRing(Ptr<Packet>sendPacket, int nodeid)
{
    InetSocketAddress remoteAddress = InetSocketAddress (m_nodeid_nodeaddr[nodeid],m_unicast_port);
    m_unicast_socket->Connect(remoteAddress);
    m_unicast_socket->Send(sendPacket);

}

void DCNET::SendAnnoucementForStar(Ptr<Packet>sendPacket, int nodeid)
{
    InetSocketAddress remoteAddress = InetSocketAddress (m_nodeid_nodeaddr[nodeid],m_unicast_port);
    m_unicast_socket->Connect(remoteAddress);
    m_unicast_socket->Send(sendPacket);
...
void DCNET::GenerateAnnouncementsForStar()
{
    std::ostringstream ss;
    int bit;
    for(unsigned  round = 0; round <m_message_length ; round++)
    ...
    m_announcementMap.insert(pair<int,std::string>( m_my_nodeid,ss.str() ));
    std::string message=ss.str();
    if(m_my_nodeid !=m_my_star_nodeid)
    {
        NS_LOG_DEBUG("Sending announcement for node " << m_my_star_nodeid << "from node " << m_my_nodeid << "\n");
        NS_LOG_DEBUG("Message is " << ss.str() <<"\n");
        Ptr<Packet> sendPacket =Create<Packet> ((uint8_t*)message.c_str(),message.size());
        MyTag sendTag;
        sendTag.SetSimpleValue( MESSAGE_DCNET_ANOUNCEMENT);
        sendPacket->AddPacketTag(sendTag);
        Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable> ();
        double jitter = uv->GetValue(0.01,0.07);
        Simulator::Schedule (Seconds(jitter),&DCNET::SendAnnoucementForStar,this,sendPacket,m_my_star_nodeid);
    }
}
...
void DCNET::GenerateAnnouncementsForRing(std::string previous_result)
{
    ...
    if(m_immediate_neighbor!=m_my_nodeid)
    {
        ...
        NS_LOG_DEBUG("sending the anouncement for the node " << m_immediate_neighbor << " and annoucement is" << ss.str() );
        Simulator::Schedule (Seconds(0.05),&DCNET::SendAnnouncementForRing,this,ss.str(),m_immediate_neighbor);
    }
    else
    {

        m_final_message=ss.str();
        m_final_announcement=true;
        std::cout << "Message " << ss.str() <<"will be sent as final message by node " << m_my_nodeid <<"\n";
        ...
        Simulator::Schedule (Seconds(0.01),&DCNET::SendFinalAnnoucementForRing,this,sendPacket,m_my_start_cluster_node);
        m_dcnet_start=false;
    }
...
void  DCNET::BroadcastAnnouncementsForStar()
{
    std::ostringstream ss;
    for(unsigned  round = 0; round <m_message_length ; round++)
    {
    ...
    while ((packet = socket->RecvFrom (from)))
    {
        ...
        if(message_id==MESSAGE_DCNET_ANOUNCEMENT)
        {
            ...
            if(m_topology=="star")
            {
                ...
                if(m_announcementMap.size()==m_nodeaddr_nodeid.size()-1)
                {
                    Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable> ();
                    double jitter = uv->GetValue(0.1,0.6);
                    Simulator::Schedule(Seconds(jitter),&DCNET::BroadcastAnnouncementsForStar,this);
                }
            }
            if(m_topology=="fullyconnected")
            {
        ...
        if(message_id==MESSAGE_DCNET_FINAL_ANOUNCEMENT)
        {
            std::string recMessage = std::string((char*)buffer);
            if(!m_final_announcement)
            {
                ...
                sendTag.SetSimpleValue(MESSAGE_DCNET_FINAL_ANOUNCEMENT);
                sendPacket->AddPacketTag(sendTag);
                Simulator::Schedule (Seconds(0.01),&DCNET::SendFinalAnnoucementForRing,this,sendPacket, m_immediate_neighbor);
                if(m_is_master_node)
                {
                    if(m_num_message_repeat>0)
                    {
                        m_final_announcement=false;
                        Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable> ();
                        double jitter = uv->GetValue(6,12);
                        Simulator::Schedule(Seconds(jitter),&DCNET::SendConfirmation,this,true);
                    }
                }
            }
