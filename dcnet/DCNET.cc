...
void DCNET::GeneratePublickeys()
{
	...
	for (std::map<int,ns3::Ipv4Address>::iterator it = m_nodeid_nodeaddr.begin() ; it != m_nodeid_nodeaddr.end(); ++it){
		...
		if(m_my_nodeid !=neighbor_nodeid) {
			...
			double jitter = uv->GetValue(10,25);
...
void DCNET::GenerateAnnouncementsForRing(std::string previous_result)
{
    ...
    for(int round = 0; round < m_message_length ; round++) {
		if(m_is_master_node) {
		...
		for (std::map<int,std::string>::iterator it=m_prngstringMap.begin(); it!=m_prngstringMap.end(); ++it) {
			...
			//std::cout << "prgstring for node is " << prgsecretstring << "\n";
...
void DCNET::ReceiveCoinFlip (Ptr<Packet> recPacket,Address from)
{
	...
	if(dec.size() <3)
    {
		std::cout << "dec size is less than expected\n";
	}
    ...
    {
		//std::cout << "Received all prng strings for node " << m_my_nodeid << "\n";
		
    	//Simulator::Schedule (Seconds(0.05),&DCNET::GenerateAnnouncementsForRing,this);
	}
}
...
void DCNET::GenerateCoinFlips()
{
	...
	for (std::map<int,ns3::Ipv4Address>::iterator it = m_nodeid_nodeaddr.begin() ; it != m_nodeid_nodeaddr.end(); ++it){
		...
		if(m_my_nodeid <neighbor_nodeid) {
			 ...
			 Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable> ();
			 double jitter = uv->GetValue(100,400);
             Simulator::Schedule(Seconds(jitter),&DCNET::SendCoinFlips,this,encrypted,neighbor_nodeid);	
			
        }
    }
	//check if it received all prng strings ; if yes, then send its anouncement to nodes based on topology.
	if(m_rank==1) {
	    //no need to wait to receive anouncements and also prng strings ; send its anouncement
		Simulator::Schedule (Seconds(0.05),&DCNET::GenerateAnnouncementsForRing,this,"");
  	...
  	while ((packet = socket->RecvFrom (from)))
  	{
		...
		if(message_id==MESSAGE_DCNET_ANOUNCEMENT) {
			...
			if(m_topology=="ring") {
				...
				Simulator::Schedule (Seconds(0.05),&DCNET::GenerateAnnouncementsForRing,this,recMessage);
			}
	...
	Ptr<Packet> packet;
  	...
  	while ((packet = socket->RecvFrom (from)))
  	{
		...
		//recMessage = recMessage.substr (0,recMessage.size());
	  	...
	  	if(messageID==MESSAGE_DCNET_INITIATE) {    //Start dcnet protocol
