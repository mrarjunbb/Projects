...
void ReceiveBroadcastAnnouncement(Ptr<Socket> socket)
{
    ...
    delete buffer;
	std::cout <<"message received by node " << recNodeIndex <<"is "<< recMessage <<"\n";
}
...
//	internet.SetRoutingHelper (aodv); // has effect on the next Install ()
   ...
   // announcementPacketCounter = (numNodes * numNodes) - numNodes;
	...
	AnimationInterface anim("dcnet_net_anim_star.xml");
    ...
    monitor->SerializeToXmlFile("dcnet_star.flowmon",true,true);
